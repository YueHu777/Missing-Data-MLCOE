...
def train(output_directory,
    ...
    training_data = np.split(training_data, 500, 0)
    ...
    while n_iter < n_iters + 1:
        for batch in training_data:
            ...
            mask = tf.tile(tf.expand_dims(mask,0),[tf.shape(batch)[0],1,1])
            loss_mask = tf.cast(1-mask,tf.bool)
            batch = tf.transpose(batch, perm = [0, 2, 1])

            assert (tf.shape(batch) == tf.shape(mask)).numpy().all() & (tf.shape(batch)== tf.shape(loss_mask)).numpy().all()
