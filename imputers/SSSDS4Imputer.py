...
class Residual_block(keras.layers.Layer):
    ...
    def call(self, input_data):
        ...
        h = self.conv_layer(h)
        h = tf.transpose(self.S41(tf.transpose(h,perm=[1,0,2])),perm=[1,2,0])
        ...
        skip = tf.transpose(skip, perm=[0, 2, 1])
...
class SSSDS4Imputer(keras.models.Model):
    ...
    def call(self, input_data):
        ...
        conditional = tf.concat([conditional, tf.cast(mask,tf.float32)], axis=1)
