...
def get_initializer(name, activation=None):
    ...
    if name == 'uniform':
        ...
        initializer = partial(tf.keras.initializers.Constant, value=0)
    elif name == 'one':
        initializer = partial(tf.keras.initializers.Constant, value=1)
    else:
...
class TransposedLinear(keras.layers.Layer):
    ...
    def __init__(self, d_input, d_output, bias=True):
        ...
        initializer = tf.keras.initializers.HeUniform()
        self.weight = initializer(shape=(d_output, d_input),dtype = tf.float32) # nn.Linear default init

        if bias:
            bound = 1 / math.sqrt(d_input)
            initializer = tf.keras.initializers.RandomUniform(-bound, bound)
            self.bias = initializer(shape=(d_output, 1),dtype = tf.float32)
        else:
            self.bias = 0.0
...
def LinearActivation(
    ...
    if bias and zero_bias_init:
        tf.keras.initializers.Zeros(linear.bias)

    # Weight norm
    if weight_norm:
        linear = tfa.layers.WeightNormalization(linear)
...
def rank_correction(measure, N, rank=1, dtype=tf.float32):
    ...
    if measure == 'legs':
        assert rank >= 1
        P = tf.expand_dims(tf.sqrt(.5+tf.range([N], dtype=dtype)),0) # (1 N)
    elif measure == 'legt':
        assert rank >= 2
...
def nplr(measure, N, rank=1, dtype=tf.float32):
    ...
    if measure == 'random':
        ...
        w = tf.complex(-tf.exp(tf.random.normal([N//2])), tf.random.normal([N//2]))
    ...
    AP = A + tf.reduce_sum(tf.expand_dims(P,-2)*tf.expand_dims(P,-1), axis=-3)
    ...
    V_inv = tf.transpose(tf.math.conj(V))
...
class SSKernelNPLR(keras.layers.Layer):
    ...
    def _omega(self, L, dtype, device, cache=True):
        ...
        omega = tf.math.pow(omega ,tf.cast(tf.range(0, L // 2 + 1),dtype=dtype))
        # omega = omega.to(device)
        z = 2 * (1 - omega) / (1 + omega)
        if cache:
            omega = tf.Variable(_c2r(omega))
            z = tf.Variable(_c2r(z))
        return omega, z
    ...
    def __init__(
        ...
        C = tf.broadcast_to(C,tf.broadcast_static_shape(C.shape, tf.TensorShape([1, self.H, self.N]))) # (H, C, N)
        ...
        if self.L is not None:
            self.omega, self.z = self._omega(self.L, dtype=C.dtype, device=C.device, cache=True)
        ...
        self.log_dt = self.register("log_dt", log_dt, trainable.get('dt', train), lr, 0.0)
        self.B = self.register("B", _c2r(B), trainable.get('B', train), lr, 0.0)
        self.P = self.register("P", _c2r(P), trainable.get('P', train), lr, 0.0)
        if self.hurwitz:
            log_w_real = tf.log(-w.real + 1e-3) # Some of the HiPPO methods have real part 0
            w_imag = w.imag
            self.log_w_real = self.register("log_w_real", log_w_real, trainable.get('A', 0), lr, 0.0)
            self.w_imag = self.register("w_imag", w_imag, trainable.get('A', train), lr, 0.0)
            self.Q = None
        else:
            self.w = self.register("w", _c2r(w), trainable.get('A', train), lr, 0.0)
            # self.register("Q", _c2r(P.clone().conj().resolve_conj()), trainable.get('P', train), lr, 0.0)
            Q = _resolve_conj(P)
            self.Q = self.register("Q", _c2r(Q), trainable.get('P', train), lr, 0.0)
    ...
    def call(self, state=None, rate=1.0, L=None):
        ...
        w = w * tf.cast(tf.expand_dims(dt,-1),w.dtype)  # (H N)
        ...
        # else:
        r = cauchy_slow(v, z, w)
        r = r * tf.cast(dt[None, None, :, None],r.dtype)  # (S+1+R, C+R, H, L)
    ...
    def _setup_linear(self):
        ...
        D = tf.math.reciprocal(2.0/ tf.cast(tf.expand_dims(dt,-1), w.dtype) - w) # (H, N)
        R = tf.eye(self.rank, dtype=w.dtype) + 2*tf.cast(tf.math.real(contract('r h n, h n, s h n -> h r s', Q, D, P)),w.dtype)# (H r r)
        ...
        self.step_params = {
            ...
            "Q": Q, # (r H N)
            "B": B, # (1 H N)
            "E": 2.0 / tf.cast(tf.expand_dims(dt,-1),w.dtype) + w, # (H N)
        }
...
class S4(keras.layers.Layer):
    ...
    def __init__(
        ...
        dropout_fn = tf.keras.layers.SpatialDropout2D if self.transposed else tf.nn.Dropout
    ...
    def call(self, u, **kwargs): # absorbs return_output and transformer src mask
        ...
        if not self.transposed: y = tf.transpose(y,[0,2,1])
...
class S4Layer(keras.layers.Layer):
    ...
    def __init__(self, features, lmax, N=64, dropout=0.0, bidirectional=True, layer_norm=True):
        ...
        self.norm_layer = tf.keras.layers.LayerNormalization(-1) if layer_norm else tf.identity
